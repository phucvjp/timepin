...
const DBPMap: React.FC = () => {
  ...
  const bases = [
    ...
    { 
      ...
      status: 'destroyed', 
      phase: 1,
      description: 'Cứ điểm phía đông, bảo vệ sân bay Mường Thanh',
    ...
    { 
      ...
      status: 'destroyed', 
      phase: 2,
      description: 'Cứ điểm phía bắc, nơi diễn ra trận đánh ác liệt nhất',
    ...
    { 
      ...
      status: 'destroyed', 
      phase: 3,
      description: 'Cứ điểm cuối cùng, nơi địch đầu hàng',
  ...
  return (
    <section className="py-16 md:py-20 bg-gradient-to-b from-slate-800 to-slate-900">
      <div className="container mx-auto px-2 md:px-10 max-w-7xl">
        ...
        {/* Map Container */}
        <div className="relative bg-gradient-to-br from-green-900/40 to-brown-900/40 backdrop-blur-md rounded-3xl p-12 md:p-16 border border-yellow-500/30 shadow-2xl mb-12">
          <div className="relative aspect-video bg-gradient-to-br from-green-800/50 to-brown-800/50 rounded-2xl overflow-hidden border-2 border-yellow-600/50">
            {/* Mountains Background */}
            <svg className="absolute inset-0 w-full h-full opacity-30" viewBox="0 0 100 100" preserveAspectRatio="none">
              <polygon points="0,70 20,40 40,60 60,30 80,50 100,60 100,100 0,100" fill="currentColor" className="text-green-900" />
              <polygon points="10,80 30,50 50,70 70,45 90,65 100,75 100,100 0,100" fill="currentColor" className="text-green-800" />
            </svg>

            {/* River */}
            <svg className="absolute inset-0 w-full h-full opacity-20" viewBox="0 0 100 100" preserveAspectRatio="none">
              <path d="M0,85 Q25,80 50,85 T100,85 L100,100 L0,100 Z" fill="currentColor" className="text-blue-600" />
            </svg>
            ...
            {bases.map((base) => (
              ...
              >
                ...
                <motion.div
                  ...
                  className="absolute -bottom-10 left-1/2 -translate-x-1/2 whitespace-nowrap text-base md:text-lg font-bold bg-black/70 px-4 py-2 rounded-full border border-white/20"
                >
                  {base.name}
                </motion.div>
            ...
            {/* Phase Indicator */}
            <div className="absolute top-4 left-4 bg-black/70 backdrop-blur-md rounded-lg px-6 py-3 border border-white/20">
          ...
          <div className="mt-12 flex justify-center gap-12">
            <div className="flex items-center gap-4">
              ...
              <span className="text-base md:text-lg text-gray-300">Cứ điểm địch</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="w-6 h-6 bg-red-500 border-2 border-red-400 rounded-full" />
              <span className="text-base md:text-lg text-gray-300">Đã tiêu diệt</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="w-6 h-6 bg-yellow-500 border-2 border-yellow-400 rounded-full" />
              <span className="text-base md:text-lg text-gray-300">Chiến thắng</span>
            </div>
          </div>
        </div>
          ...
          {selectedBase && (
            ...
            >
              <motion.div
                ...
                className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-10 max-w-lg w-full border border-white/20 shadow-2xl"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex justify-between items-start mb-8">
                  <h3 className="text-3xl md:text-4xl font-bold text-white">{selectedBase.name}</h3>
                  <motion.button
                    whileHover={{ scale: 1.1 }}
                    whileTap={{ scale: 0.9 }}
                    onClick={() => setSelectedBase(null)}
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-gray-300 text-lg">{selectedBase.date}</span>
                  </div>

                  <div className="flex items-center gap-4">
                    <Users className="w-6 h-6 text-red-400" />
                    <span className="text-gray-300 text-lg">{selectedBase.casualties}</span>
                  </div>

                  <div className="flex items-center gap-4">
                    <Target className="w-6 h-6 text-blue-400" />
                    <span className="text-gray-300 text-lg">Giai đoạn {selectedBase.phase + 1}</span>
                  </div>

                  <p className="text-gray-200 leading-relaxed mt-6 text-lg">
